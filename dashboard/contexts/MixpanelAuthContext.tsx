...
export function MixpanelAuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkAuth = useCallback(async () => {
    try {
      ...
      const res = await fetch("/api/mixpanel/projects");
      if (res.ok) {
  ...
  const connect = useCallback(() => {
    ...
    window.location.href = "/api/mixpanel/auth";
  }, []);

  const logout = useCallback(async () => {
    await fetch("/api/mixpanel/logout", { method: "POST" });
    setAuthState("idle");
    setProjects([]);
  ...
  const selectProject = useCallback(async (project: MixpanelProject) => {
    ...
    try {
      const res = await fetch(`/api/mixpanel/schema?projectId=${project.id}`);
      if (res.ok) {
